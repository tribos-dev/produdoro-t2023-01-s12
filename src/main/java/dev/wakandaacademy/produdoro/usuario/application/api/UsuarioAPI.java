...
public interface UsuarioAPI {
	...
	@ResponseStatus(code = HttpStatus.NO_CONTENT)
	void alteraStatusUsuarioPausaLonga(@RequestHeader(name = "Authorization", required = true) String token,
			@PathVariable UUID idUsuario);

	@PatchMapping(value = "/{idUsuario}/foco")
	@ResponseStatus(code = HttpStatus.NO_CONTENT)
	void patchAlteraStatusParaFoco(@RequestHeader(name = "Authorization", required = true) String token,
			@PathVariable UUID idUsuario);
}
