...
public class DataHelper {
	...
	public static Usuario getUsuarioForAtivaTarefa() {
		return Usuario.builder().idUsuario(ID_USUARIO_VALIDO).email(TOKEN_VALIDO).configuracao(null)
				.status(StatusUsuario.FOCO).quantidadePomodorosPausaCurta(3).build();
	}

	public static Tarefa getTarefaForAtivaTarefa() {
		return Tarefa.builder().contagemPomodoro(2).descricao("teste concluído")
				.statusAtivacao(StatusAtivacaoTarefa.ATIVA)
				.idUsuario(UUID.fromString("b713162f-20a9-4db9-a85b-90cd51ab18f5")).idArea(UUID.randomUUID()).build();
	}
}